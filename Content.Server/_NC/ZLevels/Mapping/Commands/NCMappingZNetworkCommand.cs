...
public sealed partial class NCMappingZNetworkCommand : LocalizedEntityCommands
{
    ...
    public override string Description => "Load NCZLevelMapPrototype as ZNetwork for mapping";


    public override CompletionResult GetCompletion(IConsoleShell shell, string[] args)
    {
        var options = new List<CompletionOption>();
        foreach (var map in _proto.EnumeratePrototypes<NCZLevelMapPrototype>())
        {
            options.Add(new CompletionOption(map.ID));
        }

        return CompletionResult.FromHintOptions(options, "NCZLevelMapPrototype");
    }
    ...
    public override void Execute(IConsoleShell shell, string argStr, string[] args)
    {
        ...
        //Get Map Prototype
        if (!_proto.Resolve<NCZLevelMapPrototype>(args[0], out var indexedZMap))
        {
            shell.WriteError($"Unknown NCZLevelMapPrototype {args[0]}");
            return;
        }
