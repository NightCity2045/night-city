...
/// <summary>
/// Universal z-grid network recalculator driven by <see cref="NCZGridConnectorComponent"/>.
...
/// </summary>
public sealed partial class NCZGridConnectorSystem : EntitySystem
{
    ...
    public override void Initialize()
    {
        ...
        SubscribeLocalEvent<NCZGridConnectorComponent, MapInitEvent>(OnConnectorMapInit);
        SubscribeLocalEvent<NCZGridConnectorComponent, AnchorStateChangedEvent>(OnConnectorAnchorChanged);
        SubscribeLocalEvent<NCZGridConnectorComponent, EntityTerminatingEvent>(OnConnectorTerminating);

        SubscribeLocalEvent<NCZGridComponent, EntityTerminatingEvent>(OnGridTerminating);
        SubscribeLocalEvent<TileChangedEvent>(OnTileChanged);
        SubscribeLocalEvent<GridSplitEvent>(OnGridSplit);

        SubscribeLocalEvent<NCZGridNetworkComponent, ComponentShutdown>(OnGridNetworkShutdown);
    }

    private void OnConnectorMapInit(Entity<NCZGridConnectorComponent> ent, ref MapInitEvent args)
    {
        _dirty = true;
    }

    private void OnConnectorAnchorChanged(Entity<NCZGridConnectorComponent> ent, ref AnchorStateChangedEvent args)
    {
        _dirty = true;
    }

    private void OnConnectorTerminating(Entity<NCZGridConnectorComponent> ent, ref EntityTerminatingEvent args)
    {
        _dirty = true;
    }

    private void OnGridTerminating(Entity<NCZGridComponent> ent, ref EntityTerminatingEvent args)
    {
        _dirty = true;
    }
    ...
    private void OnGridNetworkShutdown(Entity<NCZGridNetworkComponent> ent, ref ComponentShutdown args)
    {
    ...
    /// minimal delta: survivors keep their pose, only joined grids fire <see cref="NCGridAddedIntoZNetworkEvent"/>
    /// and only departed grids fire <see cref="NCGridRemovedFromZNetworkEvent"/>. Networks are reused (never
    ...
    private void RecalculateGridNetworks()
    {
        ...
        _removeBuffer.Clear();
        var nq = EntityQueryEnumerator<NCZGridNetworkComponent>();
    ...
    private void ComputeDesiredComponents()
    {
        ...
        var query = EntityQueryEnumerator<NCZGridConnectorComponent, TransformComponent>();
