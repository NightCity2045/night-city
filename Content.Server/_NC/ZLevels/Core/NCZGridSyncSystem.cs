...
///
/// Geometry model: each network designates one <see cref="NCZGridNetworkComponent.AnchorGrid"/>
/// (static planet if present, otherwise the lowest <see cref="EntityUid"/> for determinism). Every
/// other grid stores its pose as <see cref="NCZGridComponent.NetworkOffset"/>/
/// <see cref="NCZGridComponent.NetworkRotation"/> relative to that anchor. The anchor is the single
...
/// </summary>
public sealed partial class NCZGridSyncSystem : VirtualController
{
    [Dependency] private SharedTransformSystem _transform = default!;
    [Dependency] private NCZLevelsSystem _zlevels = default!;
    [Dependency] private NCZGridConnectorSystem _connectorSystem = default!;

    [Dependency] private EntityQuery<NCZGridComponent> _gridCompQuery = default!;
    ...
    public override void Initialize()
    {
        ...
        SubscribeLocalEvent<NCZGridComponent, NCGridAddedIntoZNetworkEvent>(OnGridLinked);
        SubscribeLocalEvent<NCZGridComponent, NCGridRemovedFromZNetworkEvent>(OnGridUnlinked);

        SubscribeLocalEvent<NCZGridComponent, MoveEvent>(OnGridMoved);
        SubscribeLocalEvent<NCZGridComponent, MassDataChangedEvent>(OnMassChanged);
    }

    private void OnGridLinked(Entity<NCZGridComponent> zGridEnt, ref NCGridAddedIntoZNetworkEvent ev)
    {
    ...
    private void OnGridUnlinked(Entity<NCZGridComponent> ent, ref NCGridRemovedFromZNetworkEvent ev)
    {
    ...
    private void OnGridMoved(Entity<NCZGridComponent> ent, ref MoveEvent ev)
    {
    ...
    private void OnMassChanged(Entity<NCZGridComponent> ent, ref MassDataChangedEvent args)
    {
    ...
    private void ApplyAnchorToGrid(EntityUid gridUid, NCZGridComponent comp, Vector2 anchorPos, Angle anchorRot)
    {
    ...
    private EntityUid ChooseAnchor(NCZGridNetworkComponent net)
    {
    ...
    private bool HasValidAnchor(NCZGridNetworkComponent net)
    {
    ...
    private void EnsureAnchor(NCZGridNetworkComponent net)
    {
    ...
    /// </summary>
    private void RebaseToAnchor(NCZGridNetworkComponent net, EntityUid newAnchor)
    {
    ...
    private void RecalculateNetworkCache(Entity<NCZGridNetworkComponent> network)
    {
        var totalMass = 0f;
        var hasStatic = false;
    ...
    private void SnapNewGridToAnchor(NCZGridNetworkComponent net, Entity<NCZGridComponent> grid)
    {
    ...
    private void RunRigidBodyConsensus(NCZGridNetworkComponent net)
    {
    ...
    public override void UpdateBeforeSolve(bool prediction, float frameTime)
    {
        ...
        var netEnum = EntityQueryEnumerator<NCZGridNetworkComponent>();
    ...
    private void CorrectFlyingNetwork(NCZGridNetworkComponent net)
    {
        RunRigidBodyConsensus(net);
