...
namespace Content.Server._NC.ZLevels.LaddersCache;

public sealed partial class NCZLevelsLaddersCacheSystem : EntitySystem
{
    ...
    [Dependency] private EntityQuery<NCZLevelsLaddersCacheComponent> _cacheQuery = default!;

    public override void Initialize()
    {
        base.Initialize();

        SubscribeLocalEvent<NCZLevelHighGroundComponent, MapInitEvent>(OnLadderInit);
        SubscribeLocalEvent<NCZLevelHighGroundComponent, ComponentShutdown>(OnLadderShutdown);
        SubscribeLocalEvent<NCZLevelHighGroundComponent, AnchorStateChangedEvent>(OnLadderAnchorChanged);
    }

    private void OnLadderInit(Entity<NCZLevelHighGroundComponent> ent, ref MapInitEvent args)
    {
        TryCacheLadder(ent);
    }

    private void OnLadderShutdown(Entity<NCZLevelHighGroundComponent> ent, ref ComponentShutdown args)
    {
        TryRemoveCache(ent);
    }

    private void OnLadderAnchorChanged(Entity<NCZLevelHighGroundComponent> ent, ref AnchorStateChangedEvent args)
    {
        if (args.Anchored)
            TryCacheLadder(ent);
        else
    ...
    /// </summary>
    private void TryCacheLadder(Entity<NCZLevelHighGroundComponent> ent)
    {
        ...
        var cache = EnsureComp<NCZLevelsLaddersCacheComponent>(gridUid.Value);
        ...
        cache.Slopes[tilePos] = new NCCachedSlope
        {
            Entity = ent,
            Direction = dir,
        };
    ...
    /// </summary>
    private void TryRemoveCache(Entity<NCZLevelHighGroundComponent> ent)
    {
        if (!_xformQuery.TryGetComponent(ent, out var xform))
            return;
