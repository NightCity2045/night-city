...
using Content.Client._NC.ZLevels.Core.Overlays;
...
namespace Content.Client._NC.ZLevels.Core;

/// <summary>
/// Only process Eye offset and drawdepth on clientside
/// </summary>
public sealed partial class NCClientZLevelsSystem : NCSharedZLevelsSystem
{
    ...
    {
        base.Initialize();
        _overlay.AddOverlay(new NCZLevelBlurOverlay());

        SubscribeLocalEvent<NCZPhysicsComponent, ComponentStartup>(OnStartup);
        SubscribeLocalEvent<NCZPhysicsComponent, GetEyeOffsetEvent>(OnEyeOffset);
    }

    private void OnEyeOffset(Entity<NCZPhysicsComponent> ent, ref GetEyeOffsetEvent args)
    {
    ...
    private void OnStartup(Entity<NCZPhysicsComponent> ent, ref ComponentStartup args)
    {
    ...
    {
        base.Shutdown();
        _overlay.RemoveOverlay<NCZLevelBlurOverlay>();
    }
}
...
/// </summary>
internal sealed partial class NCClientZLevelsPreAnimSystem : EntitySystem
{
    [Dependency] private SpriteSystem _sprite = default!;
    [Dependency] private EntityQuery<MapGridComponent> _mapGridQuery = default!;
    [Dependency] private EntityQuery<NCZPhysicsComponent> _zPhysQuery = default!;
    ...
    public override void FrameUpdate(float frameTime)
    {
        ...
        var query = EntityQueryEnumerator<NCZPhysicsComponent, SpriteComponent>();
        ...
        while (syncQuery.MoveNext(out var uid, out _, out var sprite, out var xform))
        {
            ...
            if (!_zPhysQuery.TryComp(parent, out var parentZPhys))
                continue;
            var zOffset = new Vector2(0, parentZPhys.LocalPosition * NCSharedZLevelsSystem.ZLevelOffset);
...
/// </summary>
internal sealed partial class NCClientZLevelsPostAnimSystem : EntitySystem
{
    [Dependency] private SpriteSystem _sprite = default!;
    ...
    public override void FrameUpdate(float frameTime)
    {
        ...
        var query = EntityQueryEnumerator<NCZPhysicsComponent, SpriteComponent, TransformComponent>();
        while (query.MoveNext(out var uid, out var zPhys, out var sprite, out var xform))
        {
            var rawZ = new Vector2(0, zPhys.LocalPosition * NCSharedZLevelsSystem.ZLevelOffset);
            Vector2 zOffset;
