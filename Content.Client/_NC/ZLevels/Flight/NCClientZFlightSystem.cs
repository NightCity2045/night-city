...
using Content.Client._NC.ZLevels.Core;
...
namespace Content.Client._NC.ZLevels.Flight;

public sealed partial class NCClientZFlightSystem : NCSharedZFlightSystem
{
    ...
    public override void Update(float frameTime)
    {
        ...
        var query = EntityQueryEnumerator<NCZFlyerComponent, NCZPhysicsComponent, TransformComponent, SpriteComponent>();
        while (query.MoveNext(out var uid, out var flyer, out var zPhys, out var xform, out var sprite))
        {
            ...
            if (flyer.FlightVfx is not null)
            {
                ...
                _sprite.SetOffset(vfx, new Vector2(0, zPhys.LocalPosition * NCClientZLevelsSystem.ZLevelOffset) + zPhys.SpriteOffsetDefault);
            }
        }
    }
}
