...
namespace Content.Shared._NC.ZLevels.Damage;

public sealed partial class NCZLevelDamageSystem : EntitySystem
{
    ...
    private static readonly EntProtoId FallVFX = "NCDustEffect";

    public override void Initialize()
    {
        base.Initialize();

        SubscribeLocalEvent<PhysicsComponent, NCZLevelHitEvent>(OnFallDamage);

        _config.OnValueChanged(CCVars.NCBaseFallingDamage, i => BaseFallingDamage = i, true);
        _config.OnValueChanged(CCVars.NCBaseFallingOtherDamage, i => BaseFallingOtherDamage = i, true);
        _config.OnValueChanged(CCVars.NCBaseFallingStunTime, i => BaseFallingStunTime = i, true);
        _config.OnValueChanged(CCVars.NCBaseFallingOtherStunTime, i => BaseFallingOtherStunTime = i, true);
    }

    private void OnFallDamage(Entity<PhysicsComponent> ent, ref NCZLevelHitEvent args)
    {
        var damageModifier = 1f;
        var stunModifier = 1f;

        var damageToOtherEv = new NCZFallingOnTargetDamageCalculateEvent(args.ImpactPower);
        ...
        var damageToSelfEv = new NCZFallingDamageCalculateEvent(ent, args.ImpactPower);
        ...
        var imFallOnEv = new NCZImFallOnEvent(entitiesAround, args.ImpactPower);
        ...
        foreach (var victim in entitiesAround)
        {
            ...
            var editDamageToSelfEv = new NCZFallingDamageCalculateEvent(ent, args.ImpactPower);
            ...
            var fellOnMeEv = new NCZFellOnMeEvent(ent, args.ImpactPower);
            ...
            if (otherDamage > 0)
            {
                var otherDmgSpec = new DamageSpecifier(_prototypes.Index(PhysicalDamageType), (int) otherDamage);
                _damageable.TryChangeDamage(victim, otherDmgSpec, origin: ent);
            }
        }
        ...
        if (damageAmount > 0)
        {
            var selfDmgSpec = new DamageSpecifier(_prototypes.Index(PhysicalDamageType), (int) damageAmount);
...
/// </summary>
public sealed partial class NCZFallingDamageCalculateEvent(EntityUid fallen, float speed) : EntityEventArgs
{
...
/// </summary>
public sealed partial class NCZFallingOnTargetDamageCalculateEvent(float speed) : EntityEventArgs
{
    public float DamageMultiplier = 1;
    public float StunMultiplier = 1;
    public float Speed = speed;
...
/// </summary>
public sealed partial class NCZImFallOnEvent(HashSet<EntityUid> targets, float speed) : EntityEventArgs
{
...
/// </summary>
public sealed partial class NCZFellOnMeEvent(EntityUid fallen, float speed) : EntityEventArgs
{
    public EntityUid Fallen = fallen;
    public float Speed = speed;
}
