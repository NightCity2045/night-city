...
using Content.Shared._NC.ZLevels.Core.Events;
...
public abstract partial class NCSharedZLevelsSystem
{
    ...
    {
        SubscribeLocalEvent<NCZLevelViewerComponent, MoveEvent>(OnViewerMove);
        SubscribeLocalEvent<NCZLevelViewerComponent, NCToggleZLevelLookUpAction>(OnToggleLookUp);
    }

    protected virtual void OnViewerMove(Entity<NCZLevelViewerComponent> entity, ref MoveEvent args)
    {
        ...
        DirtyField(entity, entity.Comp, nameof(NCZLevelViewerComponent.LookUp));
    }

    private void OnToggleLookUp(Entity<NCZLevelViewerComponent> entity, ref NCToggleZLevelLookUpAction args)
    {
        if (args.Handled)
            return;
        ...
        if (HasOpaqueAbove(entity))
        {
            _popup.PopupClient(Loc.GetString("nc-zlevel-look-up-fail"), entity, entity);
            return;
        }
    ...
    public bool HasOpaqueAbove(EntityUid ent, Entity<NCZMapComponent?>? currentMapUid = null)
    {
    ...
    /// World-position overload; see also <see cref="HasOpaqueAbove(EntityUid, Entity{NCZMapComponent?}?)"/>.
    /// </summary>
    [PublicAPI]
    public bool HasOpaqueAbove(Vector2 worldPos, Entity<NCZMapComponent?> currentMap)
    {
        ...
        float maxSourceDistanceFromOpeningEdgeTiles = float.PositiveInfinity)
    {
        ...
        {
            if (_map.TryGetTileRef(openingMap, grid, tile, out var tileRef) && !NCZLevelOpeningCache.IsOpeningTile(tileRef.Tile, TilDefMan))
                return false;
