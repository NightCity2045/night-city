...
public abstract partial class NCSharedZLevelsSystem
{
    ...
    {
        SubscribeLocalEvent<NCZPhysicsComponent, NCZLevelMapMoveEvent>(OnZLevelMapMove);
        SubscribeLocalEvent<NCZPhysicsComponent, MoveEvent>(OnMoveEvent);
        SubscribeLocalEvent<NCZMapComponent, TileChangedEvent>(OnTileChanged);
    }
    ...
    /// <returns></returns>
    [PublicAPI]
    public float DistanceToGround(Entity<NCZPhysicsComponent?> target)
    {
    ...
    private void OnTileChanged(Entity<NCZMapComponent> ent, ref TileChangedEvent args)
    {
    ...
    private void RequestCacheMovement(Entity<NCZPhysicsComponent> entity, bool force = true)
    {
    ...
    private void OnMoveEvent(Entity<NCZPhysicsComponent> entity, ref MoveEvent args)
    {
    ...
    private void OnZLevelMapMove(Entity<NCZPhysicsComponent> ent, ref NCZLevelMapMoveEvent args)
    {
        ent.Comp.CurrentZLevel = args.CurrentZLevel;
        DirtyField(ent, ent.Comp, nameof(NCZPhysicsComponent.CurrentZLevel));
    ...
    /// </summary>
    private float ComputeGroundHeightInternal(Entity<NCZPhysicsComponent?> target, out bool stickyGround, int maxFloors = 1)
    {
        stickyGround = false;
        ...
        Entity<NCZMapComponent> checkingMap = (xform.MapUid.Value, zMapComp);
    ...
    /// </summary>
    [PublicAPI]
    public bool HasTileAbove(EntityUid ent, Entity<NCZMapComponent?>? currentMapUid = null)
    {
    ...
    /// </summary>
    [PublicAPI]
    public bool HasTileAbove(Vector2i indices, Entity<NCZMapComponent?> map)
    {
    ...
    /// World-position overload; see also <see cref="HasTileAbove(EntityUid, Entity{NCZMapComponent?}?)"/>
    /// and <see cref="HasTileAbove(Vector2i, Entity{NCZMapComponent?})"/>.
    /// </summary>
    [PublicAPI]
    public bool HasTileAbove(Vector2 worldPos, Entity<NCZMapComponent?> currentMap)
    {
    ...
    [PublicAPI]
    public void SetZPosition(Entity<NCZPhysicsComponent?> ent, float newPosition)
    {
        if (!Resolve(ent.Owner, ref ent.Comp))
            return;

        ent.Comp.LocalPosition = newPosition;
        DirtyField(ent, ent.Comp, nameof(NCZPhysicsComponent.LocalPosition));
        WakeBody(ent);
    }

    [PublicAPI]
    public void UpdateGravityState(Entity<NCZPhysicsComponent?> ent)
    {
        if (!Resolve(ent.Owner, ref ent.Comp))
            return;

        var ev = new NCCheckGravityEvent();
        RaiseLocalEvent(ent.Owner, ref ev);

        SetZGravity(ent, ev.Gravity);
    }

    private void SetZGravity(Entity<NCZPhysicsComponent?> ent, float newGravityMultiplier)
    {
        if (!Resolve(ent.Owner, ref ent.Comp))
            return;

        ent.Comp.GravityMultiplier = newGravityMultiplier;
        DirtyField(ent, ent.Comp, nameof(NCZPhysicsComponent.GravityMultiplier));
        WakeBody(ent);
    }
    ...
    /// </summary>
    [PublicAPI]
    public void SetZVelocity(Entity<NCZPhysicsComponent?> ent, float newVelocity)
    {
        if (!Resolve(ent.Owner, ref ent.Comp))
            return;

        ent.Comp.Velocity = newVelocity;
        DirtyField(ent, ent.Comp, nameof(NCZPhysicsComponent.Velocity));
        WakeBody(ent);
    }
    ...
    /// </summary>
    [PublicAPI]
    public void AddZVelocity(Entity<NCZPhysicsComponent?> ent, float newVelocity)
    {
    ...
    [PublicAPI]
    public bool TryMove(EntityUid ent, int offset, Entity<NCZMapComponent?>? map = null)
    {
        ...
        var ev = new NCZLevelMapMoveEvent(offset, targetMap.Comp.Depth);
...
[ByRefEvent]
public struct NCZLevelMapMoveEvent(int offset, int level)
{
    /// <summary>
...
/// </summary>
[ByRefEvent]
public struct NCZLevelFallMapEvent;
...
[ByRefEvent]
public struct NCZLevelHitEvent(float impactPower)
{
    /// <summary>
...
/// </summary>
[ByRefEvent]
public struct NCGetZVelocityEvent(Entity<NCZPhysicsComponent> target)
{
    public Entity<NCZPhysicsComponent> Target = target;
...
/// </summary>
[ByRefEvent]
public struct NCCheckGravityEvent()
{
