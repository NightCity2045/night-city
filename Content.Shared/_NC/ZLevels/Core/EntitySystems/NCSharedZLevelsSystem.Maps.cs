...
public abstract partial class NCSharedZLevelsSystem
{
    ...
    /// </summary>
    [PublicAPI]
    public bool TryGetMapNetwork(EntityUid mapUid, out Entity<NCZMapNetworkComponent> zLevel)
    {
        zLevel = default;
        ...
        // The component is initialized before the server assigns its network.
        if (!networkUid.IsValid())
            return false;
    ...
    [PublicAPI]
    public bool TryMapOffset(Entity<NCZMapComponent?> entity, int offset, out Entity<NCZMapComponent> output)
    {
        return TryMapOffset(entity, offset, out output, out _);
    }

    [PublicAPI]
    public bool TryMapOffset(Entity<NCZMapComponent?> entity, int offset, out Entity<NCZMapComponent> output, [NotNullWhen(true)] out MapComponent? mapComponent)
    {
        output = default;
        mapComponent = null;
    ...
    [PublicAPI]
    public bool TryMapUp(Entity<NCZMapComponent?> inputMapUid, out Entity<NCZMapComponent> aboveMapUid) =>
        TryMapOffset(inputMapUid, 1, out aboveMapUid);

    [PublicAPI]
    public bool TryMapDown(Entity<NCZMapComponent?> inputMapUid, out Entity<NCZMapComponent> belowMapUid) =>
    ...
    /// </summary>
    [PublicAPI]
    public List<EntityUid> GetAllMapsAbove(Entity<NCZMapComponent> entity)
    {
    ...
    /// </summary>
    [PublicAPI]
    public void GetAllMapsAbove(Entity<NCZMapComponent> entity, List<EntityUid> result)
    {
        result.Clear();
    ...
    /// </summary>
    [PublicAPI]
    public List<EntityUid> GetAllMapsBelow(Entity<NCZMapComponent> entity)
    {
    ...
    private Entity<NCZMapComponent>? MapOffset(Entity<NCZMapComponent?> entity, int offset)
    {
